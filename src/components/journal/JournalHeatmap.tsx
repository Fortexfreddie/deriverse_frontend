  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto flex items-start justify-center px-4 md:px-6 pb-4">
        <div className="w-full max-w-[600px] grid grid-cols-7 gap-1 md:gap-1.5">
          {['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'].map(day => (
            <div key={day} className="text-[8px] md:text-[9px] text-center text-muted-foreground font-bold mb-1 opacity-40">
              {day}
            </div>
          ))}

          {Array.from({ length: emptyDays }).map((_, i) => (
            <div key={`empty-${i}`} className="aspect-square opacity-5 border border-dashed border-border/20" />
          ))}
