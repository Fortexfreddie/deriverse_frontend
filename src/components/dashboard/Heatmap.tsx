...
export function Heatmap({ data = {}, year, month, className }: HeatmapProps) {
  const currentDate = new Date()
  const displayYear = year || currentDate.getFullYear()
  const displayMonth = month !== undefined ? month : currentDate.getMonth()

  const days = useMemo(() => {
    const start = startOfMonth(new Date(displayYear, displayMonth))
    ...
    <div className={cn("flex flex-col", className)}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">
            Performance Heatmap <span className="text-primary font-bold ml-2">{format(new Date(displayYear, displayMonth), 'MMM yyyy').toUpperCase()}</span>
        </h2>
